...
const COMMUNITY_ID = 'network-community-community-id' + Date.now()
...
test('receive event from community title update', async done => {
  expect.assertions(2)
  const almostDone = wrapUp(done, 2)
...
test('receive event from nickname update', async done => {
  expect.assertions(2)
  const almostDone = wrapUp(done, 2)
...
test('receive event from publication post', async done => {
...
test('receive event from peer that left', async done => {
  expect.assertions(1)
  const community1 = await store1.communities.get(COMMUNITY_ID)
  community1.once('peer', async () => {
    const { nickname } = await store1.user.get()
...
test('receive events from peer that joined', async done => {
  expect.assertions(1)
